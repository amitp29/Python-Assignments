...
y = Counter(str1.split())
my_dict = {}

for times in y.iterkeys():
    x = str1.split()
    u = y[times]
    lst = []
    ...
    if(u>1):
        for i in range(u):
            indices = x.index(times)
            if(x[-1] == times):
                print 'This is the last word: ',times
            else:
                print '%s ke baad ..... %s'%(times,x[indices+1])
                lst.append(x[indices+1])
                x.remove(times)
    else:
        indices = x.index(times)
        if(x[-1] == times):
